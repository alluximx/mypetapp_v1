...
// Use aliases only if you intend to use local pats instead
const frameworkAlias = {
  ...
  '@ui-kitten/components': path.resolve(
    env.UI_KITTEN_PACKAGES_PATH,
    'components',
  ),
  '@ui-kitten/date-fns': path.resolve(env.UI_KITTEN_PACKAGES_PATH, 'date-fns'),
  '@ui-kitten/eva-icons': path.resolve(
    env.UI_KITTEN_PACKAGES_PATH,
    'eva-icons',
  ),
...
module.exports = function (api) {
  ...
  const plugins = [
    ...
    [
      ...
      {
        moduleName: 'react-native-dotenv-2',
        path: '.env',
        blacklist: null,
        whitelist: null,
        safe: true,
