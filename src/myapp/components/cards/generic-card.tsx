import {  StyleService, useStyleSheet, Text, Card, List } from "@ui-kitten/components";
...
const GenericCard = (props): React.ReactElement => {
    ...
    return (
        <Card style={[styles.cardStyle, props.styleCard]}>
            {props.data.date ? <Text style={styles.labelDate}>{props.data.date.getDate()}/{(props.data.date.getMonth() + 1)}/{props.data.date.getFullYear()}</Text> : <Text></Text>}
...
const defaultStyle = (type) => StyleService.create({
    header: {
        fontSize: 20,
        fontFamily: 'Montserrat-Bold',
        textAlign: type,
